...
//         l_complete = 8*((taille[1]/8) +1);
//         h_complete = 8*((taille[0]/8) +1);
//     }
...
//     uint16_t nbr_blocs = h_complete*l_complete/64;
...
//     int16_t*** components = decode_mcu_blocks_444(tables_dc, tables_ac, argv[1], taille_complete, comp, N);
...
// int main(int argc, char **argv){
//     uint16_t** resultat = size_picture(argv[1]);
//     table_de_huffman** dc_ac = arbre_huffman_V2(argv[1]);
//     table_de_huffman* tables_dc = dc_ac[0];
//     table_de_huffman* tables_ac = dc_ac[1];
//     uint16_t* taille = resultat[0];
//     uint16_t N = *resultat[2]; 
//     uint16_t* qt_id = resultat[3];
//     uint16_t* composantes = resultat[1];
...
//     tables_ac, argv[1], taille_complete, comp, N);
...
//     uint8_t**** blocs_final = post_idct(blocs_idct, nbr_blocs, N);
    

//     double**** blocs_RGB;
//     uint8_t**** blocs_rgb_sature;
//     if (N == 3){
//         blocs_RGB = conversion_rgb(blocs_final, nbr_blocs);
//         blocs_final = saturation_rgb(blocs_RGB, nbr_blocs);
...
//     generate_pgm_V3_troncature(taille_complete, taille, blocs_final, N);
