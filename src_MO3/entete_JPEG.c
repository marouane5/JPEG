...
uint8_t* extract_quant_table(char* path){
    FILE* fichier = fopen(path, "rb");
    ...
    void dec_to_bin(uint8_t n, char* buffer) {
        for (int i = 7; i >= 0; i--) {
        ...
        for (int j = 0; j< 16; j++){
            ...
            if (taille > 0){
                for (int i = 0; i< taille ; i++){
                    ...
                    dec_to_bin(S1.code, bin);
                    
                    printf("le code pour le symbole (DC) %02X est %s\n", S1.symbole, bin);
                    code += 1;
                    L_DC[compteur]= S1;
                    compteur += 1;
                }
        ...
        for (int j =0; j< 16; j++){
            ...
            if (taille > 0){
                for (int i = 0; i< taille; i++){
                    ...
                    printf("le code pour le symbole (AC) %02X est %s\n", S1.symbole, bin);
                    code += 1;
                    L_AC[compteur]= S1;
                    compteur += 1;
                }
