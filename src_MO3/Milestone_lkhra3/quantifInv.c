...
void free_quantif_inv(int16_t*** blocs_Y_Cb_Cr_postQ, ComponentInfo* comp, uint16_t* taille, uint16_t N){
    uint8_t H_Y = comp[0].h_samp;
    uint8_t V_Y = comp[0].v_samp;
    ...
    for (int p=0; p<N; p++){
        int nb_blocs = total_mcu * comp[p].h_samp * comp[p].v_samp;
        for (int k = 0; k < nb_blocs; k++){
            free(blocs_Y_Cb_Cr_postQ[p][k]); 
        }
        free(blocs_Y_Cb_Cr_postQ[p]);
    }
    free(blocs_Y_Cb_Cr_postQ);
}