...
table_de_huffman** arbre_huffman_V2(char* path){
    ...
    for (int k = 0; k < 2; k++){
        for (int p = 0; p < 4; p++){
            ...
            uint16_t code = 0;
            uint8_t longueur = 1;
            int compteur = 0; /* Pour savoir ou insérer dans L */    
...
int main(int argc, char **argv){
    ...
    table_de_huffman* result_DC = result[0];
    
    table_de_huffman result_DC_table0 = result_DC[0];
    int longueur = result_DC_table0.len;
    huffman_code* symboles_codes = result_DC_table0.huff_tab;

    printf("la longueur est : %d\n", longueur);
    for (int i = 0; i < longueur; i++){
        printf("la longeur est %d bits, le symbole est %02X et le code est %s\n", symboles_codes[i].longueur, symboles_codes[i].symbole, symboles_codes[i].code);
