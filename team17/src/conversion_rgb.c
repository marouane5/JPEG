...
void free_conversion_rgb(double**** blocs_RGB, int total_blocks_Y) {
    for (int i = 0; i < total_blocks_Y; i++) {
        for (int j = 0; j < 8; j++) {
            for (int k = 0; k < 8; k++) {
                free(blocs_RGB[i][j][k]);
            }
            free(blocs_RGB[i][j]);
        }
        free(blocs_RGB[i]);
...
void free_saturation_rgb(uint8_t**** blocs, int total_blocks_Y) {
    for (int i = 0; i < total_blocks_Y; i++) {
        for (int j = 0; j < 8; j++) {
            for (int k = 0; k < 8; k++) {
                free(blocs[i][j][k]);
